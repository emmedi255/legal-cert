...
export async function generatePdf({ user, condominioId, formData }) {
  let browser;
  try {
    if (isLocal) {
      // 🖥 Locale → usa Chrome installato con puppeteer
...
<body>
  ...
  <p>In ottemperanza a quanto disposto al Capo III - art. 12 del Regolamento (UE) 2016/679 relativo alla protezione dei dati personali (in seguito "Regolamento"), il condominio:</p>

  <p><strong>${intestazione.condominio_indirizzo || ""}</strong></p>
  <p><strong>${intestazione.citta || ""} ${intestazione.cap || ""} ${intestazione.provincia || ""}</strong></p>
  ...
  <p><strong>Ad eccezione per singolari casi di necessità strettamente correlati e finalizzati ad una corretta amministrazione condominiale, non saranno raccolti, detenuti, comunicati, diffusi, né trattati in alcun modo dati sensibili o giudiziari. </strong></p>
...
<dl style="margin-top: 5px; line-height: 1.2;">
 ${
   studio
     ? `
    <dt style="font-weight: bold;">Ragione sociale:</dt>
  ...
  <dd style="margin: 0 0 5px 0;">${sezione07.sedeLegale}</dd>
  ...
  <p>Il Condominio viene qualificato come <strong>Titolare del trattamento dei dati</strong> e lo stesso nomina, quale <strong>Responsabile del trattamento dei dati</strong>, l'<strong>Amministratore pro-tempore</strong>.</p>
...
`,
  ...
  `,
    ...
    const path = `pdfs/${condominioId}_informativa-privacy.pdf`;
